...
import { MyPublicationsContainer, MyPublicationsEmpty, Publications, PublicationsContainer } from "./styles";
import { getMePublications } from "../../services/publication";
...
export const MyPublications = () => {
  ...
  const fetchGetPublications = async () => {
    setLoading(true);
    const results = await getMePublications(search, categorie, initialPrice, finalPrice, (page ? page - 1 : page));
  ...
  const handlePrice = (initialPrice, finalPrice) => {
    ...
    setSearchParams({...searchParams, initialPrice, finalPrice});
  ...
  return (<MyPublicationsContainer>
